import type { Product } from '../types'
import { hsChapterForCategory } from './marketing'
import { productSlug } from '../utils/slug'

function withMeta(
  product: Omit<Product, 'slug' | 'inStock' | 'hsChapter'> & { inStock?: boolean },
): Product {
  return {
    ...product,
    inStock: product.inStock ?? true,
    hsChapter: hsChapterForCategory(product.category),
    slug: productSlug({ id: product.id, name: product.name }),
  }
}

export const MOCK_PRODUCTS: Product[] = [
  withMeta({
    id: 'prod-001',
    name: 'Sodium Hydroxide, Pellets, ACS Reagent Grade',
    casNumber: '1310-73-2',
    purity: '≥98%',
    moq: '25 kg',
    physicalState: 'Solid (pellets)',
    packaging: '25 kg drum',
    leadTime: '5–8 business days',
    estimatedPrice: 42.5,
    hazard: 'corrosive',
    sdsUrl: '#sds-naoh',
    sdsUpdatedAt: '2026-03-12',
    category: 'Inorganics',
  }),
  withMeta({
    id: 'prod-002',
    name: 'Isopropyl Alcohol, Anhydrous, HPLC Grade',
    casNumber: '67-63-0',
    purity: '≥99.9%',
    moq: '20 L',
    physicalState: 'Liquid',
    packaging: '20 L jerrican',
    leadTime: '3–5 business days',
    estimatedPrice: 18.75,
    hazard: 'flammable',
    sdsUrl: '#sds-ipa',
    sdsUpdatedAt: '2026-01-28',
    category: 'Solvents',
  }),
  withMeta({
    id: 'prod-003',
    name: 'Acetone, ACS Reagent, Low Water Content',
    casNumber: '67-64-1',
    purity: '≥99.5%',
    moq: '200 L',
    physicalState: 'Liquid',
    packaging: '200 L drum',
    leadTime: '7–10 business days',
    estimatedPrice: null,
    hazard: 'flammable',
    sdsUrl: '#sds-acetone',
    sdsUpdatedAt: '2025-11-04',
    category: 'Solvents',
    inStock: false,
  }),
  withMeta({
    id: 'prod-004',
    name: 'Hydrochloric Acid, 37% Technical Grade',
    casNumber: '7647-01-0',
    purity: '36–38%',
    moq: '1,000 L',
    physicalState: 'Liquid',
    packaging: 'IBC tote',
    leadTime: '10–14 business days',
    estimatedPrice: 0.85,
    hazard: 'corrosive',
    sdsUrl: '#sds-hcl',
    sdsUpdatedAt: '2026-02-19',
    category: 'Acids',
  }),
  withMeta({
    id: 'prod-005',
    name: 'Toluene, Industrial Grade for Coatings & Adhesives',
    casNumber: '108-88-3',
    purity: '≥99%',
    moq: '200 L',
    physicalState: 'Liquid',
    packaging: '200 L drum',
    leadTime: '4–6 business days',
    estimatedPrice: 22.1,
    hazard: 'health',
    sdsUrl: '#sds-toluene',
    sdsUpdatedAt: '2026-04-01',
    category: 'Aromatics',
  }),
  withMeta({
    id: 'prod-006',
    name: 'Hydrogen Peroxide, 35% Stabilized',
    casNumber: '7722-84-1',
    purity: '35%',
    moq: '30 L',
    physicalState: 'Liquid',
    packaging: '25 kg drum',
    leadTime: '6–9 business days',
    estimatedPrice: 31.4,
    hazard: 'corrosive',
    sdsUrl: '#sds-h2o2',
    sdsUpdatedAt: '2025-12-15',
    category: 'Oxidizers',
  }),
  withMeta({
    id: 'prod-007',
    name: 'Methanol, Absolute, ACS Spectrophotometric Grade',
    casNumber: '67-56-1',
    purity: '≥99.8%',
    moq: '20 L',
    physicalState: 'Liquid',
    packaging: '20 L jerrican',
    leadTime: '3–5 business days',
    estimatedPrice: 14.2,
    hazard: 'toxic',
    sdsUrl: '#sds-meoh',
    sdsUpdatedAt: '2026-05-22',
    category: 'Solvents',
  }),
  withMeta({
    id: 'prod-008',
    name: 'Calcium Chloride Anhydrous, Food Processing Grade',
    casNumber: '10043-52-4',
    purity: '≥94%',
    moq: '50 kg',
    physicalState: 'Solid (flakes)',
    packaging: '25 kg drum',
    leadTime: '8–12 business days',
    estimatedPrice: 9.6,
    hazard: 'irritant',
    sdsUrl: '#sds-cacl2',
    sdsUpdatedAt: '2026-02-08',
    category: 'Inorganics',
  }),
]
